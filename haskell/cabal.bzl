...
def _prepare_cabal_inputs(hs, cc, dep_info, cc_info, package_id, tool_inputs, tool_input_manifests, cabal, setup, srcs, flags, cabal_wrapper, package_database):
...
def _haskell_cabal_library_impl(ctx):
    ...
    haddock_file = hs.actions.declare_file(
        "_install/haddock/{}.haddock".format(ctx.attr.name),
        sibling = cabal,
    )
    haddock_html_dir = hs.actions.declare_directory(
        "_install/haddock_html",
        sibling = cabal,
    )
